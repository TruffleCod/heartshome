...
const noteLines = [
  ...
  `时针旋转${gap(1)}了${gap(3)}很多很多圈，没${gap(1)}有停下${gap(1)}来的意思。`,
  ...
  `不起，就主${gap(5)}动提出要私了。${gap(4)}加上和解后`,
  `面的失踪事件，${gap(1)}他也怕了，连高中都${gap(1)}没读完${gap(1)}，就把我`,
  `送出了国，生怕${gap(1)}有任何人可以联系到${gap(4)}我。“你爸`,
  `现在也是市教育${gap(1)}局的大领导呢。”我${gap(1)}说。其${gap(1)}实他应该`,
  `一直很讨厌我吧${gap(1)}。作为一个青少年心${gap(4)}理咨询领域`,
  ...
  `那张脸在${gap(4)}深褐色的液面上微微晃${gap(4)}动。“知`,
  `道我真名${gap(1)}的人${gap(1)}，只剩下你一个了。”${gap(1)}窗外${gap(1)}有人在浇`,
  `花，水管${gap(1)}里的${gap(1)}水珠打在叶片上，发出${gap(1)}细密${gap(1)}的声响。`,
  `咖啡馆里${gap(4)}有人推门进来，带进一${gap(4)}阵风，倒`,
